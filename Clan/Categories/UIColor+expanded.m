...
//返回plist文件里的颜色
+ (UIColor *)returnColorWithPlist:(NSString *)colorValue
{
    ...
    return [self getColorWithName:dic[colorValue]];
}

+ (UIColor *)returnColorWithPlist:(NSString *)colorValue withAlphaValue:(CGFloat)fvalue
{
    NSString *themePath = [[NSBundle mainBundle]pathForResource:ThemeStyle ofType:@"plist"];
    NSDictionary *dic = [NSDictionary dictionaryWithContentsOfFile:themePath];
    return [self getColorWithName:dic[colorValue] withAlphaValue:0.5];
}

//返回十六进制色值
+ (NSString *)hexValueFromUIColor:(UIColor *)color
{
...
+ (UIColor *)getColorWithName:(NSString *)name
{
    ...
    if (rgbs.count == 3) {
        ...
        float r = [rgbs[0] floatValue];//转FLOAT
...
//RGB颜色转换
+ (UIColor *)getColorWithName:(NSString *)name withAlphaValue:(CGFloat)valu
{
    if (name.length == 0) {
        return nil;
    }
    ...
    if (rgbs.count == 3) {
        ...
        float g = [rgbs[1] floatValue];
        float b = [rgbs[2] floatValue];
        UIColor *color = Color(r, g, b, valu);
        return color;
    }
    return nil;
}
