...
- (void)addLinkText:(NSString *)text linkUrlArray:(NSMutableArray *)linkUrlArray {
    ...
    for (int i = 0; i < markupCount; i++) {
        if((openMarkupRange = [styleAttributedString.mutableString rangeOfString:kLinkOpenMarkup]).location != NSNotFound) {
            ...
            NSString *tempString = [styleAttributedString.string substringWithRange:NSMakeRange(0, openMarkupRange.location)];
            NSRegularExpression *regex = [NSRegularExpression regularExpressionWithPattern:@"\\{(.*?)\\/(.*?)\\}" options:0 error:nil];
            __block NSInteger rangeLenth = 0;
            __block NSInteger imageCount = 0;
            [regex enumerateMatchesInString:tempString options:0 range:NSMakeRange(begin, [tempString length]) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
                rangeLenth = result.range.length + rangeLenth;
                imageCount ++;
            }];
            // Calculate the style range that represent the string between the open and close markups
            NSRange styleRange = NSMakeRange(openMarkupRange.location - rangeLenth + imageCount, closeMarkupRange.location - openMarkupRange.location - kLinkOpenMarkup.length);
            if (styleRange.location <= realText.length && realText.length >= styleRange.length) {
                NSString *urlString = [styleAttributedString.string substringWithRange:NSMakeRange(openMarkupRange.location + kLinkOpenMarkup.length, closeMarkupRange.location - openMarkupRange.location - kLinkOpenMarkup.length)];
                NSURL *url;
