    ...
    for (CFIndex lineIndex = 0; lineIndex < numberOfLines; lineIndex++) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns(line)) {
            ...
            if (imageName) {
                ...
                NSString *filePath = [documentPath stringByAppendingPathComponent:array[0]];
                NSData *imageData ;
                if([fileManager fileExistsAtPath:filePath]){
                    imageData = [NSData dataWithContentsOfFile:[[documentPath stringByAppendingPathComponent:array[0]] stringByAppendingPathComponent:array[1]]];
                }
                if (!isNull(imageData)) {
