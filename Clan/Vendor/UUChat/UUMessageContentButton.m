...
- (void)MenuItemCopyClicked
{
    UIPasteboard *pboard = [UIPasteboard generalPasteboard];
    for (UIView *view in self.subviews) {
        if ([view isKindOfClass:[SJRichLabel class]]) {
            SJRichLabel *label = (SJRichLabel *)view;
            pboard.string = label.text;
        }
    }
}

- (void)MenuItemDeleteClicked
{
    NSDictionary *dic = @{@"para":self.path};
    [[NSNotificationCenter defaultCenter] postNotificationName:@"CHAT_CELL_DELETE" object:nil userInfo:dic];
}
...
{
    if(action == @selector(MenuItemCopyClicked))
        return YES;
    else if(action == @selector(MenuItemDeleteClicked))
        return YES;
    else
        return NO;
}
