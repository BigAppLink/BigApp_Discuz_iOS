...
- (void)setButtonTexts:(NSMutableArray *)buttonTexts forCategory:(NSString *)category{
    ...
    NSAssert(buttonTexts != nil, @"Array containing texts to be set on buttons is nƒil");
    ...
    if (([self.buttons count] - 1) == [buttonTexts count]) {
        ...
        for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
            ...
            [btn setTitle:buttonTexts[i] forState:UIControlStateNormal];
            CGFloat imageHeight = CGRectGetHeight([self.buttons[i] frame]) - 20;
            NSData *imageData = [NSData dataWithContentsOfFile:[[documentPath stringByAppendingPathComponent:category] stringByAppendingPathComponent:buttonTexts[i]]];
            UIImage *faceimage = [UIImage imageWithData:imageData];
            [self.buttons[i] setImage:[faceimage scaledToSize:CGSizeMake(imageHeight, imageHeight) highQuality:YES] forState:UIControlStateNormal];
        }
    } else {
        ...
        for (NSUInteger i = 0; i < [buttonTexts count]; ++i) {
            ...
            NSData *imageData = [NSData dataWithContentsOfFile:[[documentPath stringByAppendingPathComponent:category] stringByAppendingPathComponent:buttonTexts[i]]];
            UIImage *faceimage = [UIImage imageWithData:imageData];
            [button setImage:[faceimage scaledToSize:CGSizeMake(imageHeight, imageHeight) highQuality:YES] forState:UIControlStateNormal];
