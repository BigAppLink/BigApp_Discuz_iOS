...
@property (strong,nonatomic) NSMutableArray *faceImageArray;
@property (strong, nonatomic) NSDictionary *faceDic;
@end
...
- (NSString *)categoryNameAtIndex:(NSUInteger)index {
    ...
    NSArray *categoryList = _faceImageArray;
...
- (NSArray *)imagesForSelectedSegments {
    ...
    dispatch_once(&onceToken, ^{
        ...
        for (int i = 0;
             i < _faceImageArray.count;
             i++) {
            NSFileManager *fileManager = [NSFileManager defaultManager];
            NSString *filePath = [documentPath stringByAppendingPathComponent:_faceImageArray[i]];
            if([fileManager fileExistsAtPath:filePath]){
                NSData *imageData = [NSData dataWithContentsOfFile:[[documentPath stringByAppendingPathComponent:_faceImageArray[i]] stringByAppendingPathComponent:_faceDic[_faceImageArray[i]][0]]];
...
- (void)facePlistwithDic{
//    _faceDic = [[NSUserDefaults standardUserDefaults]objectForKey:@"ClanFaceImage"];
    _faceDic = [UserDefaultsHelper valueForDefaultsKey:kUserDefaultsKey_ClanFaceImage];
    if (!_faceImageArray) {
        _faceImageArray = [NSMutableArray array];
    }
    NSFileManager *fm = [NSFileManager defaultManager];
    //判断沙盒下是否存在表情
    BOOL isExist = [fm fileExistsAtPath:[NSObject pathInDocumentDirectory:@"ClanFaceImage.plist"]];
    if (isExist) {
        NSDictionary *list = [ [NSDictionary alloc] initWithContentsOfFile:[NSObject pathInDocumentDirectory:@"ClanFaceImage.plist"]];
        for (NSString *key in list) {
            [_faceImageArray addObject:key];
        }
    }
}
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource{
    ...
    if (self) {
        ...
        //拿到plist映射关系
        [self facePlistwithDic];
        BOOL showBigEmotion = NO;
        // initialize category
        ...
        //        if (!showBigEmotion) {
        //            self.easeTabBar = [[UIEaseTabBar alloc] initWithFrame:CGRectMake(0, (self_Height - easeTabBar_Height + temp6p), CGRectGetWidth(self.bounds), easeTabBar_Height)
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    ...
    if (selectedIndex != _selectedIndex) {
        ...
        for (int i=0; i<_numOfTabs; i++) {
            ...
            if (i==selectedIndex) {
                [tabButton setImage:[self.selectedImages[i] transformWidth:20 height:20] forState:UIControlStateNormal];
...
- (void)configScrollView{
    ...
    for (int i=0; i<self.numOfTabs; i++) {
        ...
        scrollViewWidth+=button.width;
        [self.scrollView addSubview:button];
        [self.tabButtons addObject:button];
    }
    self.scrollView.contentSize = CGSizeMake(scrollViewWidth, self.scrollView.height);
}
