...
@interface IDMPhotoBrowser () {
	...
	UIBarButtonItem *_previousButton, *_actionButton;
...
- (id)init {
    if ((self = [super init])) {
        ...
        _isdraggingPhoto = NO;
        _savedImgsArr = [NSMutableArray new];
...
//    }
    ...
    _doneButton.imageEdgeInsets = UIEdgeInsetsMake(0, -30, 0, 0);
//    _doneButton.frame = CGRectMake(0, 0, 26, 26);
    [_doneButton setFrame:[self frameForDoneButtonAtOrientation:currentOrientation]];
    [_doneButton setImage :[UIImage imageNamed:@"nav_back"] forState:UIControlStateNormal];
    [_doneButton addTarget:self action:@selector(doneButtonPressed:) forControlEvents:UIControlEventTouchUpInside];
    
    _nextButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _nextButton.frame = CGRectMake(self.view.bounds.size.width-90, 10, 80, 22);
//    [_nextButton setTitle:@"保存图片" forState:UIControlStateNormal];
    [_nextButton setImage:kIMG(@"download_N") forState:UIControlStateNormal];
    _nextButton.imageEdgeInsets = UIEdgeInsetsMake(0, 0, 0, -30);
    [_nextButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_nextButton addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
...
//    _nextButton = [[UIBarButtonItem alloc]initWithTitle:@"保存图片" style:UIBarButtonItemStylePlain target:self action:@selector(saveImage)];
    
    
    // Counter Label
    _counterLabel = [[UILabel alloc] initWithFrame:CGRectMake(150, 0, 80, 40)];
//    _counterLabel.center = _toolbar.center;
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"k_dz_photobroswer_close" object:[NSNumber numberWithInteger:_currentPageIndex]];
}
...
//        [self.view addSubview:_doneButton];
    ...
    [_toolbar addSubview:_counterLabel];
    [self updateToolbar];
...
//    [_nextButton setImageInsets:<#(UIEdgeInsets)#>]
    IDMPhoto *photo = [self photoAtIndex:_currentPageIndex];
    if ([_savedImgsArr containsObject:photo.photoURL]) {
        [_nextButton setImage:kIMG(@"download_H") forState:UIControlStateNormal];
        return;
    }
    [_nextButton setImage:kIMG(@"download_N") forState:UIControlStateNormal];
}
...
//    UIImageView *currentImageView = _scrollView.subviews[index];
    ...
    UIImageWriteToSavedPhotosAlbum(scrollView.photoImageView.image, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
    ...
    if (error) {
        ...
        //TODO save success
        IDMPhoto *photo = [self photoAtIndex:_currentPageIndex];
        [_savedImgsArr addObject:photo.photoURL];
        [_nextButton setImage:kIMG(@"download_H") forState:UIControlStateNormal];
        label.text = @"保存成功";
    }
